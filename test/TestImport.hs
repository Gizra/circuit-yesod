...
import Types (MailType(MailTypeRegular))
...
createUser ident =
  runDB $ do
    user <-
      insertEntity
        User
        {userIdent = ident, userPassword = Nothing, userUuid = "uuid-" <> ident}
    _ <-
      insert
        Email
        { emailEmail = ident
...
createSite :: (Site -> Site) -> YesodExample App (Entity Site)
createSite func = do
  let site =
        func
          Site
          { siteName = "site1"
          , siteBackOfficeUrl = "https://example.com"
          , siteDefaultMailType = MailTypeRegular
          }
  runDB $ insertEntity site
--
-- -- | Create a Sale, and allows alterting the default values.
-- createSale :: Key User -> Text -> SaleStatus -> YesodExample App (Entity Sale)
-- createSale uid name saleStatus = do
